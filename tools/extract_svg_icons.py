#!/usr/bin/env python3
"""Extract inline SVG icons from editor index.html into individual files
icons/<name>.svg (root <svg id="icon">) and replace them with
...
for i, m in enumerate(svgs):
    ...
    # The referenced file carries the real viewBox (on its root <svg id="icon">).
    # On the outer <svg> we keep a zero-origin viewBox with the same aspect:
    # (a) an outer viewBox with a non-zero origin ("0 -960 960 960") shifts the
    # <use> viewport out of the visible region and clips the icon entirely;
    # (b) with no outer viewBox the <svg> loses its intrinsic aspect ratio and
    # falls back to the 300x150 replaced-element default instead of filling its
    # .icon container.
    ...
    out.append(f'{open_tag}<use href="icons/{NAMES[h]}.svg#icon" /></svg>')
    last = m.end()
out.append(html[last:])
open(HTML, 'w').write(''.join(out))

print(f"replaced {len(svgs)} inline SVGs with {len(order)} icon files")
for h in order:
