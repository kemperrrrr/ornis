...
8. ~~**Единый источник шейдеров: перевести render на Rust→WGSL (путь 2)**~~ — ✅ **закрыт полностью 2026-09-08/09, неактуально как план (исторический текст ниже)**
   ...
   >
   > ✅ **Белые швы-терминаторы исправлены 2026-09-12 (корень — не
   > геометрия):** `ggx_ndf_aniso` (`shaders/math.rs`, `#[kernel]` —
   > единый источник CPU/GPU) был ненормирован: потерян член `NoH²`,
   > масштаб `1/(αu²·αv²)` вместо `1/(αu·αv)` — интеграл по полусфере
   > 10135 вместо 1 (r=0.1), спекуляр завышен ~10⁴×; на терминаторах
   > `if nol <= EPS continue` отрезал конечный вклад → жёсткие X-дуги
   > 10–40 LSB на гладких сферах (r≤0.2) в обоих путях (forward и
   > deferred — общий код), rough-сферы чистые. Доказано бисекцией:
   > поворот/тесселяция не двигают дуги (не меш), один свет — одна дуга,
   > forward показывает те же дуги (не g-buffer). Исправление — форма
   > Хейтца (тесты `ggx_ndf_aniso_peak/tilted`, интеграл = 1); `continue`
   > оставлен (срез после фикса sub-LSB). Заодно чинен
   > `octahedral_decode` (WGSL + Rust-зеркало): развёртка низа была не
   > `−sign·t`, а `sign·other·t` — до ~69° ошибки на всей z<0-полусфере
   > (тест `octahedral_decode_covers_folded_hemisphere`); доку `[0,1]²`
   > → `[-1,1]²`. Golden `golden_probe_1280x720.png` перебазирован
   > (`render_probe`, Apple M1; дифф только внутри дисков сфер 4.15%,
   > фон идентичен). Хвосты проверены, удалять нечего:
   > `LightingVertexOut`/`LightingVertexOutput` уже отсутствуют (0
   > ссылок), дубля interface-режима в `WgslStruct` нет (эмиссия
   > `location()`/`builtin()` в `stages.rs` — активный путь),
   > `OpenPBRMaterial`-декод и `#[wgsl_fn]`-хелперы — действующий
   > single-source, оставлены сознательно.
