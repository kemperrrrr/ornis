...
| **S3** | месяцы | Layout из типов; `PassBuilder` → deprecated-шим; конфликт писателей — ошибка | 🟡 реализовано 2026-08-19 (шим-дока, golden-тесты, `planned_pool_bytes`); конфликт писателей снят с повестки (порядок = регистрация) |
