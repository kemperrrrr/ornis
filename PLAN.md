...
**G2b (диагностирован 2026-08-07, не решён)**: бокс на статичном полу улетает (энергетическая инъекция). Первопричина: **warm start без отдельной стадии** — накопленный импульс применяется как «скоростная добавка» каждый проход и разгоняет тело. Правильный паттерн (Box2D): стадия `WarmStart` (применить кэшированные импульсы в начале шага) + velocity solve **с нуля**, а не бесконечное накопление. Также найден и исправлен настоящий баг: манифолды получали захардкоженные `body_a/body_b=0,0` — реальные индексы пары подставляются в `detect_collisions` (фикс в коммите). Тест `box_rests_on_static_floor` помечен `#[ignore]` до решения warm-start. Осталось: box↔capsule → G2c.
