...
**G7 (2026-08-15, частично — CPU-половина реализована)**: производительность. Сделано: (1) **Замеры** через новый `examples/perf_probe.rs` (grid 16×16 стеков = 1025 тел, big_stack_32; диагностика сна per-body). Baseline: grid **220 мс/кадр** — и корень был не в солвере, а в сломанном сне: ~50% устоявшейся сцены никогда не засыпало. (2) **Фиксы сна** (каждый — с зелёными тестами): канонизация id острова в `rebuild_islands` (минимальный индекс члена вместо корня union-find, который плясал от ротации `sort_axis` в SweepAndPrune), O(n²)→HashMap asleep-пар; **гистерезис пробуждения** (спящий остров будит только удар с approach-скоростью > 0.5 м/с — микроджиттер покоя vn≈0 больше не будит, исчез sleep↔wake churn); **спящее тело = статика для солвера** (при засыпании `inv_mass=0, inertia=0`, при `wake_island` — восстановление; без этого спящие накапливали невидимую скорость и взрывались до 20 м/с); `detect_collisions` получил `asleep` и пропускает пары «оба спят»; early-out `step()` при полностью спящем мире. Итог по сну: settled grid 1024/1025 спит, **220 → 0.53 мс/кадр**. (3) **Per-island параллельный солвер** (rayon, safe Rust): `solve_contacts_velocity` переписан как оркестратор — последовательный пре-пасс sleep/wake → union-find partition по СВЕЖИМ манифолдам → gather в шарды `IslandWork` (клоны тел, манифолды с локальными индексами, глобальные ключи warm-кэша) → dispatch `par_iter_mut`/`iter_mut` (пороги ≥2 острова и ≥24 манифолда) → scatter только dynamic-тел → мердж warm-кэша по дизъюнктным ключам; позиционная стадия `solve_contacts_position(&mut islands)` переиспользует шарды после интеграции (re-gather → dispatch → scatter). Gather/scatter вместо unsafe-шаринга — сознательный выбор. Джойнты и TOI остаются последовательными (гонок нет — работают на main-массиве между стадиями). (4) **Детерминизм**: острова дизъюнктны по dynamic-телам ⇒ per-island GS бит-в-бит совпадает с глобальным; новый тест `solver_is_deterministic_across_thread_counts` (сцена 3×3×4 бокса, 120 кадров, rayon pool 1 vs 4 потока, сравнение `to_bits()` позиций/ориентаций/скоростей всех тел) — зелёный. Все 24 старых теста прошли БЕЗ изменения чисел. Итог: grid awake-фаза 220 → ~75 мс/кадр (≈3×, settle 60 кадров за 4.5 с), settled 0.276 мс/кадр; 25/25, clippy/fmt чисто. Остатки: high-stack rocking (32-стек подрагивает, 24/33 спит — вопрос качества, не производительности); джойнты и TOI последовательные; gather/scatter без пулинга аллокаций; **SIMD-wide путь и GPU-перенос не начаты** (GPU требует решения про детерминизм — см. строку таблицы).
