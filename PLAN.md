...
| **S5** | кварталы | Один scheduler: ресурсы рендера в ECS, автопараллелизм по доступам, без extract | 🟡 S5a+S5b+S5c ✅ CI (последний ffef5e9): шедулер, levels, параллельная запись, `order_before`; остаток — интеграция живого рендера с `Schedule` ядра (extract-free мир), bench числами на GPU-машине |
