...
| **S6** | годы | Решение: роспуск `RenderGraph` как структуры данных либо фиксация «почему нет» | ✅ ратифицировано 2026-08-19: реестр + `mermaid()`-проекция; полный роспуск отклонён (4 причины); ✅ подтверждено после d4 (2026-09-07): `FramePlan` удалён, `SystemSet`+`TransientPool` остаются render-aware проекцией над общим `bitset_level_plan` (3 причины — см. `docs/rendering/unified-scheduler.md`, раздел «S6 — пересмотр после d4») |
...
Пересмотр после d4 (2026-09-07): `FramePlan` удалён полностью (−806
строк), `SystemSet` — единственный реестр деклараций рендера, движок
уровней один (`ornis-schedule::bitset_level_plan` общий для
`core::Schedule` и `TransientPool`). Решение S6 подтверждено: дальнейший
роспуск `SystemSet` в `Schedule` отклонён (доменное состояние пула /
разные пространства ключей / encoder-исполнение — детали в
`docs/rendering/unified-scheduler.md`, раздел «S6 — пересмотр после
d4»). Сближение с `Schedule` — через валюту `AccessDesc` и
паритет-оракул, не через ликвидацию типа.
