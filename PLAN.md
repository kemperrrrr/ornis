...
параллельно с «a» — **S0–S1 в работе (код написан 2026-08-18, детали и
статусы — Приложение C)**. Полный план — **Приложение C** (этапы S0–S6).
...
| **S2** | месяцы | Пасс = типизированная система (`Reads`/`Writes` в типах), роспуск `match` по именам | 🟡 S2a ✅ CI; S2b (режимы) реализовано 2026-08-19, проверка CI |
| **S3** | месяцы | Layout из типов; `PassBuilder` → deprecated-шим; конфликт писателей — ошибка | ❌ |
| **S4** | месяцы | `Budget` как вход планировщика: валидация на `build()`, укладка в бюджет | ❌ |
| **S5** | кварталы | Один scheduler: ресурсы рендера в ECS, автопараллелизм по доступам, без extract | ❌ |
| **S6** | годы | Решение: роспуск `RenderGraph` как структуры данных либо фиксация «почему нет» | ❄️ |
...
- **S2b (спроектировано 2026-08-19, не реализовано)**: у
  ...
  у ~10).
- **S2b реализовано (2026-08-19, проверка CI)**: `Forward<OwnsDepth |
...
### S6 — решение о роспуске графа (§28.1, годы)

Только по итогам S5. Если типовые множества + scheduler покрывают всё,
что умел `RenderGraph` как структура данных, — тип демотируется до
отладочного экспорта (dot/mermaid для доков): граф становится проекцией
типов. Если нет — письменно фиксируем, какой кейс не покрывается, и граф
остаётся. Оба исхода — успех (§28: «если не получится — останется
работающий RenderGraph и понимание, почему»).
