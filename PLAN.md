...
| **S5** | кварталы | Один scheduler: ресурсы рендера в ECS, автопараллелизм по доступам, без extract | 🟡 S5a+S5b ✅ CI (2026-08-19, a8bb49c, все 11 стадий): шедулер ядра, levels() (lighting ∥ forward), параллельная запись (lavapipe seq==par; wasm — последовательный твин без Sync); S5c — `.before/.after`, bench |
