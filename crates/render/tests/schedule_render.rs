...
//! mirror `parallel_render.rs` (the S5b gate of the same family).

use glam::{Mat4, Quat, Vec3};
use ornis_render::render_backend::RenderContext;
use ornis_render::{InstanceData, OpenPBRMaterial, RenderFrame3D, Technique};
...
fn read_back(device: &wgpu::Device, queue: &wgpu::Queue, texture: &wgpu::Texture) -> Vec<u8> {
    ...
    let buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("schedule readback buffer"),
        size: (padded * SIZE) as u64,
        usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
        mapped_at_creation: false,
    });
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("schedule readback encoder"),
    });
...
fn schedule_ordered_render_matches_sequential_pixels() {
    ...
    let renderer = ornis_render::Renderer3D::new(&device, &surface_config, 1);
    ...
    let (sched_tex, sched_view) = target(&device, "schedule target");

    // Sequential: the default graph path (single borrowed encoder).
    ...
    // Schedule-driven: the E1 projection orders the same passes as core
    // `Schedule` systems; recording still goes through the borrowed
    // encoder (`render_schedule`).
    let mut sched = RenderFrame3D::new_with(
        wgpu::TextureFormat::Rgba8Unorm,
        (SIZE, SIZE),
        Technique::Hybrid,
        true,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("schedule encoder"),
    });
    sched.render_schedule(
        RenderContext {
            device: &device,
            queue: &queue,
            encoder: &mut encoder,
    ...
    let sched_pixels = read_back(&device, &queue, &sched_tex);
    assert_eq!(
        seq_pixels, sched_pixels,
