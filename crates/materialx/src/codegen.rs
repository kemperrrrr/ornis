...
use crate::parser::{Input, MaterialXDocument, Node, NodeDef, NodeGraph};
...
impl<'a> GraphEvaluator<'a> {
    ...
    fn eval_layer(
        ...
        let top = inputs
            .get("top")
            .ok_or(CodegenError::InputNotFound("top".into()))?;
        let base = inputs
            .get("base")
            .ok_or(CodegenError::InputNotFound("base".into()))?;
    ...
    fn get_float(
        ...
        match inputs.get(key) {
            ...
            Some(OutputValue::String(s)) => s
                .parse()
                .map_err(|_| CodegenError::TypeConversion("string to float".into())),
    ...
    fn parse_constant(&self, value: &str, _type: &str) -> Result<OutputValue, CodegenError> {
        ...
        if value.contains(',') {
            ...
            if parts.len() == 3 {
                let r = parts[0]
                    .parse::<f32>()
                    .map_err(|_| CodegenError::TypeConversion("parse float".into()))?;
                let g = parts[1]
                    .parse::<f32>()
                    .map_err(|_| CodegenError::TypeConversion("parse float".into()))?;
                let b = parts[2]
