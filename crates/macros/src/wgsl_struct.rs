...
//! must be a scalar (`f32`, `u32`, `i32`, `bool`), a fixed-size array of one
...
fn wgsl_field_layout(
    ...
    if let syn::Type::Array(arr) = ty {
        ...
        return Err(syn::Error::new(
            arr.elem.span(),
            "WgslStruct: unsupported array element type; use f32/u32/i32/bool or a struct with a WGSL layout",
        ));
    }
    ...
    Err(syn::Error::new(
        ty.span(),
        "WgslStruct: unsupported field type; use f32/u32/i32/bool, fixed-size arrays of them, [[f32; 4]; 4], or a struct with a WGSL layout",
    ))
}
