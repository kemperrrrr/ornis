...
use syn::{
    parse_macro_input, Data, DeriveInput, Fields, FieldsNamed, Generics, Ident, Type,
...
fn extract_field_info(struct_name: &Ident, fields: &FieldsNamed) -> Vec<FieldInfo> {
    fields.named.iter()
        ...
        .filter_map(|(idx, f)| {
            ...
            Some(FieldInfo { name, ty, lane_ty, wrapper_name })
        })
        .collect()
}
...
fn extract_pack_info(input: &DeriveInput) -> Result<PackInfo, syn::Error> {
    ...
    let fields = match &input.data {
        Data::Struct(s) => match &s.fields {
            ...
            _ => return Err(syn::Error::new_spanned(&s.fields, "#[derive(Pack)] requires named fields")),
        },
        _ => return Err(syn::Error::new_spanned(&struct_name, "#[derive(Pack)] only works on structs")),
    };
    
    let field_infos = extract_field_info(&struct_name, fields);
    if field_infos.is_empty() {
        return Err(syn::Error::new_spanned(&struct_name, "#[derive(Pack)] requires at least one field"));
    }
...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let (pack_mut_struct, pack_mut_methods) = 
        generate_pack_mut_struct(struct_name, &info.fields, &ty_generics, where_clause, &pack_mut_name);
...
fn generate_pack_get(struct_name: &Ident, fields: &[FieldInfo], ty_generics: &syn::TypeGenerics) -> TokenStream2 {
