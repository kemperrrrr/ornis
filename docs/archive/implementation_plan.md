...
> ⚠️ **Стратегический поворот (июль 2026):** нативный UI-крейт заморожен. Редактор теперь работает в браузере. См. [`STRATEGY_PIVOT.md`](./STRATEGY_PIVOT.md).
...
**⚠️ Статус: заморожена (июль 2026).** Активная разработка нативного UI остановлена. Редактор переехал в браузер (см. [`STRATEGY_PIVOT.md`](./STRATEGY_PIVOT.md)). Крейт `crates/ui` остаётся в репозитории, но новые фичи не добавляются до зрелости `blitz`/`boa_engine`.

**Цель**: рендерить HTML/CSS через GPU, а JS-логику выполнять внутри движка. Поддержка OpenPBR и MaterialX для материалов.

| # | Задача | Технологии | Критерий завершения | Статус |
|---|--------|------------|---------------------|--------|
| 5.1 | Парсинг HTML/CSS в Rust + интеграция Servo | `html5ever`, `lightningcss`, `taffy`, `servo-core` (опционально) | ✅ через `html5ever` + `lightningcss` + `taffy` — парсинг HTML и CSS, flexbox layout. **Заморожено.** |
| 5.1a | Интеграция Servo как библиотеки | `servo`, `webrender` | ❄️ Отложена до зрелости экосистемы |
| 5.2 | Векторный рендеринг + текст (SVG + UI) | `vello`, `skrifa`, `usvg`, `resvg` | ✅ Реализовано, но не используется в активном режиме |
| 5.3 | Встроенный JS-интерпретатор | `boa_engine` | ✅ `JsRuntime` работает, но Boa не зрел для production UI. **Заморожено.** |
| 5.4 | Headless DOM (шим для JS) | `boa_engine`, прототипное наследование | ✅ Сохранено в `crates/ui` |
| 5.5 | Связь JS ↔ Rust (Sparse Sets) | `boa_engine` + `SmartStore` | ✅ `EcsBridge` сохранён |
| 5.6 | Двухсторонний IPC (UI-поток ↔ игровой поток) | `crossbeam-channel`, `UiCommand` / `GameEvent` | ✅ Переиспользуется для связи браузер ↔ движок |
| 5.7 | In-Game Editor | Vello overlay, keyboard toggle | ❄️ Заменён на Remote Editor в браузере |
| 5.8 | **Удалённый редактор (Web)** — **основной режим** | `tiny_http`, `serde_json`, REST API | ✅ HTTP-сервер (tiny_http, порт 3420) + REST API. **Теперь это основной способ работы с редактором.** |
| 5.9 | **Система материалов: OpenPBR Surface** ✅ | `wgpu`, WGSL, `glam`, `bytemuck` | Активно в `crates/render` |
| 5.10 | **Поддержка MaterialX** | `materialx` crate / кастомный парсер `.mtlx` в WGSL | Активно в `crates/materialx` |
| 5.11 | **Рендер-пайплайн материалов** | Deferred + Forward hybrid | Активно в `crates/render` |

### Браузерный редактор (текущий фокус)

Вместо нативного UI сейчас работает следующая схема:

1. `cargo run --features editor-only` запускает только HTTP-сервер (порт 3420).
2. Разработчик открывает `http://127.0.0.1:3420` в браузере.
3. Редактор — обычное веб-приложение (HTML/CSS/JS), загружаемое с сервера.
4. Связь с движком: REST API (`/api/status`, `/api/events`, `/api/command`).
5. В перспективе: WebSocket + WASM-модуль для рендеринга сцены в `<canvas>` через WebGPU.
