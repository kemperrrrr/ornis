...
Конкретные функции, где мутация приводит к зависанию на 60s:

- `mul_inv_inertia` — `*` → `+` или `/` приводит к зависанию
- `k_entry` — замена `f32` константы на `1.0` или `-1.0` приводит к зависанию
- `solve_small`, `apply_impulse`, `solve_normal_block` — знаковые мутации
  в итерационных циклах → бесконечный цикл
- `solve_continuous` (строка 1631) — `*` → `/` в CCD-проходе
- `solve_scalar_friction` (строки 2582, 2617, 2620) — зависание на делении

Это значит, что **код не защищён от деградированных входов**:
NaN, infinity, нулевая инерция, деление на ноль уходят в бесконечный
цикл. В продакшен-движке такие входы приходят от:

- повреждённого save-файла (битый float, missing field)
- race condition между solver'ом и integrator'ом
- численной нестабильности на edge-case'ах (масса → 0, dt → 0,
  velocity → infinity после нескольких step'ов с экспоненциальным
  ростом из-за бага)
- просто плохих данных от других систем (анимация, скриптинг)
