# Мутант-тикеты — ornis-physics, исторический прогон 2026-08-24

> **Статус:** архивный отчёт о первом большом прогоне; цифры ниже
> относятся к исходному снимку и не являются текущим статусом всех
> тикетов. T13 закрыт целевым прогоном из `t13-followups.md`, а T14
> закрыт тестами в коммите `38ca753`; T8–T12 и хвосты остаются
> известными ограничениями/кандидатами на отдельные раунды.
>
> Источник: `cargo mutants --package ornis-physics --timeout 60`
> (исторический verify-heavy workflow, ручной запуск): **2351 мутант** —
...
| T13 | Hardening публичных API физики (timeout'ы) | engine.rs | 33 timeout в исходном прогоне | **высокий** | ✅ закрыт целевым прогоном |
| T14 | `RigidBody::set_orientation` (no-op) + мелочи | body.rs | 2 missed в исходном прогоне | **высокий** | ✅ закрыт (38ca753) |
...
## Где смотреть в историческом артефакте
