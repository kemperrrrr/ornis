# T13 Round 2 — итог целевого прогона timeout'ов

> **Статус:** завершённая историческая заметка. Реализация и
> локальная проверка этого раунда выполнены; текст ниже сохраняет
> ход расследования и исправленный диагноз для воспроизводимости.

> 🔴 **ДИСКЛЕЙМЕР:** исходный черновик содержал **ложный диагноз** природы
> timeout-мутаций (детали ниже и в «РЕВЬЮ-ЗАМЕТКИ» в конце). Все
> утверждения вида «assert'ы ловят / закрывают таймауты» — **ложь**.

> ⚠️ Нижеследующие команды и commit-процедура — исторический
> журнал, их не следует выполнять буквально в текущей ветке.
>
