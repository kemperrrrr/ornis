...
## 7. Открытые вопросы — закрыты по факту реализации (2026-09-07)

- [x] Отдельный крейт `render_graph` не понадобился: граф живёт
  модулями внутри `crates/render` (`frame_exec` / `frame_passes` /
  `transient_pool` / `schedule_bridge`), внешних зависимостей у этой
  части нет — исходный выбор «модуль» подтвердился.
- [x] Immediate подтверждён: `RenderFrame3D` собирается на кадр
  (`new_with` + проекция), persistent — только `Renderer3D`,
  transient-пул и кеши layout/уровневого плана (S1, hardening);
  пересборка плана при изменении сцены ничего не инвалидирует.
- [x] Пул — по полной спецификации текстуры: `TextureSpec` (формат +
  размер с resolve от surface) и есть ключ переиспользования
  (`transient_pool.rs`), а не «только текстуры одного размера».
- [x] Суб-пасс отложен: wgpu/WebGPU не даёт Vulkan-стиля
  input-attachments, вводить понятие под несуществующую возможность
  рано; вернуться при появлении реального паттерна (например,
  MSAA-tiles).
- [x] Дамп реализован иначе и общим средством: S6 — `mermaid()`
  (`ornis-schedule`, общий проектор уровневого плана для обоих
  планировщиков, пинится тестами) — PR-ревью рендерит диаграмму
  нативно; отдельный JSON-dump графа не понадобился.
