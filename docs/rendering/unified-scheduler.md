...
- **`GraphLayout::levels()`** — те же уровни для пассов рендера, из
  ...
  (`independent_branches_share_levels`), текущий конвейер — честная
  строгая цепочка (`production_graph_is_a_strict_chain`: 9 пассов, все
  уровни-синглтоны). Вывод готов; параллельная запись команд (по
  encoder'у на пасс) включается, когда в графе появятся независимые
  ветви — S5b.
