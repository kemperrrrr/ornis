...
### 3. OpenPBR Material — `crates/render/src/material.rs`
...
### 7. Headless DOM / UI — `crates/ui/`
- **Документация**: [key_ideas.md:168-180](key_ideas.md#L168-L180), [implementation_plan.md:139-148](implementation_plan.md#L139-L148)
- **Код**: `html5ever`, `lightningcss`, `taffy` (layout), `boa_engine` (JS), `vello` (рендеринг), IPC через `crossbeam-channel`
...
### 9. Runtime CPU/GPU диспетчер — `crates/core/src/dispatcher.rs`
- **Документация**: [key_ideas.md:92-105](key_ideas.md#L92-L105), [implementation_plan.md:93-104](implementation_plan.md#L93-L104)
- **Код**: `Dispatcher`, `SmartDispatcher`, `CpuExecutor`, `GpuExecutor`, `ExecutionTarget`, `Dispatcher::decide(element_count)`, тесты

### 10. Command-Based Sync foundation — `crates/core/src/command_sync.rs`
- **Документация**: [key_ideas.md:136-146](key_ideas.md#L136-L146), [implementation_plan.md:103](implementation_plan.md#L103)
- **Код**: `CommandSync`, `CommandQueue`, `GpuCommand`, `CommandType`, `ResidencyTracker`, `DataResidency`, `CommandSyncable`

### 11. PhysicsEngine trait + Builtin — `crates/core/src/physics/`
- **Документация**: [key_ideas.md:359-371](key_ideas.md#L359-L371), [implementation_plan.md:43](implementation_plan.md#L43)
- **Код**: `PhysicsEngine` trait, `BuiltinPhysicsEngine` (Sweep-and-Prune + PBD solver + raycast)

### 12. RenderBackend trait — `crates/render/src/render_backend.rs`
- **Документация**: [key_ideas.md:373-384](key_ideas.md#L373-L384), [implementation_plan.md:199](implementation_plan.md#L199)
- **Код**: `RenderBackend` trait, `Renderer3D` implementation через factory `create_render_backend`

### 13. In-Game Editor с Gizmo — `crates/ui/src/editor.rs`
- **Документация**: [implementation_plan.md:148](implementation_plan.md#L148)
- **Код**: `EditorOverlay` (hierarchy panel, inspector panel, add component modal), `GizmoSystem` (translate/rotate/scale modes, raycast, manipulation)
...
|---|---|---|---|
| **CPU/GPU автодиспетчер** | [key_ideas.md:92-105](key_ideas.md#L92-L105), [implementation_plan.md:93-104](implementation_plan.md#L93-L104) | `Dispatcher`, `SmartDispatcher`, `CpuExecutor`, `GpuExecutor` в `dispatcher.rs` | Полностью реализован: `decide(element_count)`, `CpuExecutor::execute`, `SmartDispatcher::execute_read`, тесты |
| **Command-Based Sync** | [key_ideas.md:136-146](key_ideas.md#L136-L146), [implementation_plan.md:103](implementation_plan.md#L103) | `CommandSync`, `CommandQueue`, `GpuCommand`, `ResidencyTracker`, `DataResidency` в `command_sync.rs` | **CPU-side только**: нет GPU исполнения команд (wgpu compute shader execution) |
...
- **Но**: В `editor.rs` теперь **ЕСТЬ** `EditorOverlay` с hierarchy panel, inspector panel, add component modal, `GizmoSystem` (translate/rotate/scale, raycast, manipulation) — **частично реализовано**
...
1. **Command-Based Sync GPU исполнение** — исполнение команд на GPU (wgpu compute shaders)

### Приоритет 2: Довести до mind-соответствия
2. **Remote Editor редактирование** (WebSocket + синхронизация состояния)
3. **Реализовать `compile_warning!` в `smart_pipeline`** — обучение разработчика
...
- Убрать ✅ у Remote Editor, DSP на GPU, Batch API, Линтер
...
```
Реализовано полностью:           14/25 (56%)
Частично / требует доработки:     4/25 (16%)
Заявлено ✅, но отсутствует:      6/25 (24%)
Не начато (фазы 6-11):            1/25  (4%)
```

> **Вывод**: Ядро (Sparse Sets, макросы, OpenPBR, WGSL, UI/DOM, MaterialX, RenderBackend, PhysicsEngine, Runtime CPU/GPU диспетчер, Command-Based Sync foundation, In-Game Editor с gizmos) — production-ready. Command-Based Sync GPU исполнение, Remote Editor редактирование, линтер, DSP на GPU — требуют реализации. Документация опережает код на ~1 фазу.