...
| Frame Plan (бывш. Render Graph; модули `frame_plan.rs`/`frame_exec.rs`, rename от 2026-08-23): `RenderFrame3D` + `Technique` (forward/deferred/hybrid как конфигурация плана) + блум-каскад; роспуск оболочки (2026-09-06, стадия 1): hot-path снапшот layout — `Arc<FrameLayout>` в `FrameExecutor::ensure_layout` (ключ — `FramePlan::generation`), мёртвые `FramePlan::execute`/`PassContext` удалены, `PassBuilder` — только тест/паритет-воронка (`SystemSet` — единственный prod-путь) | ✅ |
