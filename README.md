...
| `crates/ui` | Нативный UI (html5ever + lightningcss + taffy + vello + boa) | ❄️ Заморожен |
| `crates/ui-blitz` | Эксперимент с Dioxus Blitz | ❄️ Заморожен |
| `crates/ui-gosub` | Эксперимент с Gosub (план в `docs/archive/GOSUB_INTEGRATION.md`) | ❄️ Заморожен |
| `forks/` | Локальные форки зависимостей (path-deps) | Служебное |
...
| Нативный UI-крейт (`crates/ui`, vello/DOM/boa) | ❄️ | заморожен по решению о пивоте; код сохранён |
...
7. **Браузерный редактор.** Нативный UI-движок заморожен: доведение его до
   production сопоставимо с командой браузерного движка. Редактор — веб-приложение,
   сцена — WASM/WebGPU в `<canvas>`. Код `crates/ui` сохранён на случай зрелости
   blitz/boa.
